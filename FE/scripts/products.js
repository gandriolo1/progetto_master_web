  ...
  try {
    ...
    filteredProducts = allProducts.filter((product) => {
      const matchesCategoria =
        filters.categoria === "Tutto" ||
        product.CATEGORIA === filters.categoria;
      const matchesMateriale =
        filters.materiale === "Tutto" ||
        product.MATERIALE === filters.materiale;
      const matchesColore =
        filters.colore === "Tutto" || product.COLORE === filters.colore;
...
async function addToWishlist(idProdotto) {
  ...
  try {
    const response = await fetch(
      "http://localhost:4000/wishlist/aggiungiPreferito",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
