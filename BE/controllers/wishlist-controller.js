...
exports.createWish = (req, res) => {
  ...
  Wishlist.createWish(idUtente, idProdotto).then(whish => {
    res.status(200).json({ messaggio: "Prodotto aggiunto alla Whishlist", whish: whish });
  }).catch(err => {
...
exports.getWishlist = (req, res) => {
  ...
  Wishlist.getAllWish(idUtente).then(rows => {
    res.status(200).json(rows.map(row => ({ ...row, IMMAGINE: `http://localhost:4000/file/${row.IMMAGINE}` })));
  }).catch(err => {
    res.status(500).json({ errore: err.message });
  });
};
...
exports.deleteWish = (req, res) => {
  ...
  Wishlist.removeWish(idUtente, idProdotto).then(updatedWishlist => {
